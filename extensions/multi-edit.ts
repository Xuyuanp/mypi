...
function parsePatch(patchText: string): PatchOperation[] {
    ...
    while (i <= lastContentLine) {
        ...
        if (line.startsWith("*** Update File: ")) {
            ...
            if (
                i <= lastContentLine &&
                lines[i].trim().startsWith("*** Move to: ")
            ) {
                throw new Error(
...
export default function (pi: ExtensionAPI) {
    pi.registerTool({
        ...
        async execute(toolCallId, params, signal, _onUpdate, ctx) {
            ...
            const preflightTool = createEditTool(ctx.cwd, {
                operations: createVirtualEditOperations(),
            });
            ...
            for (let i = 0; i < edits.length; i++) {
                ...
                const edit = edits[i];
                try {
                    await preflightTool.execute(
                        `${toolCallId}_preflight_${i}`,
                        edit,
                        signal,
                    );
